...
### 5. Link Skills to Commands, Rules, and Subagents

When you create a **skill** that implements a **command** (e.g. the workflow for `/task` or `/build`), the skill must **link** to the command and to any rules or subagents it uses.

**Requirement**: In the skill body, add a **Related** section (e.g. at the end or under Constraints) with:

- **Command**: The slash command that invokes this skill — `.cursor/commands/{command-name}.md`.
- **Rules** (if any): Rules that apply when this skill runs — e.g. `.cursor/rules/global-rules.mdc`.
- **Subagents** (if any): Subagents this skill delegates to — e.g. `.cursor/agents/planner.md`.

**Example** (skill for `/task`):
...
- **Subagent (Plan mode)**: `.cursor/agents/planner.md` — use this structure for plans.
```

If the skill is standalone, omit the command link or list which commands might reference it.
...
- [ ] If the skill implements a command: body includes a **Related** section linking to that command, and to rules/subagents it uses (see "Link Skills to Commands, Rules, and Subagents" above).
