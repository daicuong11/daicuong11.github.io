...
### Step 5: Link Subagents to Commands and Skills

When the subagent is created for use with a **command** (e.g. invoked during `/task` Plan mode or by a skill), the subagent body must **link** to that command and to the skill that uses it.

**Requirement**: At the end of the subagent body, add a **Related** section with:

- **Command**: The slash command that may invoke or use this subagent — `.cursor/commands/{command-name}.md` (e.g. `.cursor/commands/task.md`).
- **Skill** (if any): The skill that delegates to or follows this subagent — `.cursor/skills/{skill-name}/SKILL.md` (e.g. `.cursor/skills/task/SKILL.md`).

**Example** (subagent used by `/task` in Plan mode):
...
If the subagent is standalone (not tied to a specific command), you may reference which commands or skills typically use it (e.g. "Can be invoked after /task or when the user asks for a code review").
...
5. **Link to command and skill**: When the subagent is used by a command/skill, add a **Related** section in the body (see Step 5 above).
